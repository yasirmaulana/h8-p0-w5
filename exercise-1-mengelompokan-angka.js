...
function mengelompokkanAngka(arr) {
  var kumpulanKelompok = []; kelompokGenap = []; kelompokGanjil = []; kelompokKelipatan3 = []

  for(var i=0; i<arr.length; i++){

    vkelipatan3 = arr[i]%3
    vGanjilGenap = arr[i]%2
    if(vkelipatan3 === 0){
      kelompokKelipatan3.push(arr[i])
    }
    else if(vGanjilGenap === 0){
      kelompokGenap.push(arr[i])
    }
    else if(vGanjilGenap === 1){
      kelompokGanjil.push(arr[i])
    }

  }

  return kumpulanKelompok = [kelompokGenap, kelompokGanjil, kelompokKelipatan3]
}
